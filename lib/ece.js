...
export function decryptStreamRange (secretKey, offset, length, totalEncryptedLength, rs = RECORD_SIZE) {
  ...
  // Last record needed to decrypt the range
  ...
  let endsPrematurely = true
  if (dataEnd >= totalEncryptedLength) {
    endsPrematurely = false
  ...
  return {
    ...
    decrypt: (streams) => {
      ...
      const plaintextStream = transformStream(
        encryptedStream,
        new ECETransformer(MODE_DECRYPT, secretKey, rs, null, startRecord, endsPrematurely)
      ).readable
...
class ECETransformer {
  constructor (mode, secretKey, rs, salt, seek, endsPrematurely) {
    ...
    this.rs = rs
    this.salt = salt
    this.seek = seek // sequence number of first data record
    this.endsPrematurely = endsPrematurely
  ...
  async flush (controller) {
    if (this.prevChunk) {
      await this.transformPrevChunk(!this.endsPrematurely, controller)
    }
  }
}
